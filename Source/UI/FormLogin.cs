...
namespace UI
{
    public partial class FormLogin : Form
    {
        ...
        private void FromLogin_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                txtServerURL.Text = Method.GetWebConfig(ConfigName.WebServicesURL);
            }
            catch (Exception ex)
            {
            ...
            else
            {
                ...
                try
                {
                    ...
                    string result = GetServerLatestFilesMD5();
                    if (result.Length > 0)
                    {
                        //服务器文件列表
        ...
        private void TestWebService()
        {
            ...
            try
            {
                ...
                if (strResoult.Trim() == "WanGang")
                {
                    ...
                    GlobalVal.ServicesURL = txtServerURL.Text.Trim();
                }
            }
            catch (Exception ex)
            {
        ...
        private static string GetServerLatestFilesMD5()
        {
            string strRet = string.Empty;
            WebRequest req = WebRequest.Create(GlobalVal.ServicesURL + "/GetFilesMD5.ashx");
            WebResponse res = req.GetResponse();
            System.IO.Stream resStream = res.GetResponseStream();
            Encoding encode = System.Text.Encoding.Default;
            StreamReader readStream = new StreamReader(resStream, encode);
